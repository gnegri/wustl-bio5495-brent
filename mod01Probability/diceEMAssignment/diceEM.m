...
diceEM[sample_, maxIterations_, accuracy_]:=
	Module[{numFaces, draws, binCounts, old, new, iter=0, return,
...
updateProbs[binCounts_, oldType1Prob_, oldType2Prob_, oldFaceProbs1_, oldFaceProbs2_] :=
	Module[{posteriorType1, posteriorType2, type1Count, type2Count, faceCounts1, faceCounts2, 
		draws, rollsPerDraw, newType1Prob, newType2Prob, newFaceProbs1, newFaceProbs2},
		...
		rollsPerDraw = Total[binCounts[[1]]];
		type1Count   = draws*posteriorType1;
		faceCounts1  = rollsPerDraw*draws*oldFaceProbs1;
		faceCounts2  = rollsPerDraw*draws*oldFaceProbs2;
		
		(* Finally, use these counts to compute maximum likelihood estimates for the parameters 
			and return these estimates in a list.*)
		newType1Prob  = Total[type1Count]/(draws*rollsPerDraw);
